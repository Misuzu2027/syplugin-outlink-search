<script lang="ts">
  ...
  import { MenuItem } from "@/libs/siyuan/Menu";
  import { BlockItem, DocumentItem } from "@/models/outlink-model";
  import { blockItemsSort } from "@/service/outlink/OutLinkService";
  import { blockSortSubMenu } from "@/service/search/search-util";
  ...
  function executeDocItemAction(
    ...
    let swapDocItemClickLogic =
      SettingService.ins.SettingConfig.swapDocItemClickLogic;
  ...
  async function documentItemContextmenuEvent(
    ...
    window.siyuan.menus.menu.append(
      new MenuItem({
        ...
        type: "submenu",
        submenu: blockSortSubMenu(documentItem, blockSortSubMenuCallback),
      }).element
    );
    window.siyuan.menus.menu.append(
      new MenuItem({
  ...
  async function blockSortSubMenuCallback(
    documentItem: DocumentItem,
    sortMode: ContentBlockSortMode
  ) {
    const startTime = performance.now(); // 记录开始时间

    isSearching++;
    await blockItemsSort(documentItem.subItems, sortMode, documentItem.index);
    documentItemSearchResult = documentItemSearchResult;
    isSearching--;
    const endTime = performance.now(); // 记录结束时间
    const executionTime = endTime - startTime; // 计算时间差
    console.log(
      `排序类型 : ${sortMode} , 数量 : ${documentItem.subItems.length} , 消耗时长 : ${executionTime} ms`
    );
  }
</script>
    ...
    <div>
      {#each item.subItems as subItem (subItem.block.id)}
        ...
        >
          ...
          <span class="b3-list-item__text">{@html subItem.block.content} </span>

          {#if subItem.block.name}
            <span class="b3-list-item__meta fn__flex" style="max-width: 30%">
